...
const PostMsg = forwardRef(({image, comment, name, time, comId, id, userId, ownerId}, ref) => {
    ...
    return (
        <div ref={ref} className="postMsg flex gap-4 mb-4">
            {image.length === 1 ? <i className={`w-14 h-14 rounded-full border-0 cursor-pointer bg-blue-500 text-white text-3xl font-semibold flex items-center justify-center not-italic overflow-hidden normal-case`}>{image}</i> : <img src={image} alt="images" className={` w-16 h-16 rounded-full object-cover cursor-pointer border-0`}/>}
